...
export default function AdminDashboard() {
  ...
  const handleEditExam = (exam: Exam) => {
    setEditingExam(exam); setExamTitle(exam.title); setExamSubject(exam.subject || ''); setExamDesc(exam.description || ''); setExamDuration(exam.duration)
  ...
  const resetQuestionForm = () => {
    setShowQuestionForm(false); setEditingQuestion(null); setQText(''); setQA(''); setQB(''); setQC(''); setQD(''); setQE(''); setQCorrect('A'); setQExplanation('')
  }
  ...
  const handleWordImport = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const data = await res.json()
      if (res.ok) { toast.success(`${data.count || 0} soal berhasil diimpor dari Word`); fetchQuestions(selectedExam.id); fetchStats() }
  ...
  const handleExportResults = (examId?: string) => {
    ...
    const examTitle = examId && examId !== 'all'
      ? exams.find(e => e.id === examId)?.title || 'Ujian'
      : 'Semua-Ujian'
  ...
  const filteredResults = results.filter((r) => {
    const matchSearch = r.exam.title.toLowerCase().includes(resultSearch.toLowerCase()) ||
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="bg-white/80 backdrop-blur-sm shadow-sm rounded-xl h-auto p-1.5 gap-1">
            ...
            <TabsTrigger value="guru" className="rounded-lg px-4 py-2 text-sm data-[state=active]:bg-blue-600 data-[state=active]:text-white data-[state=active]:shadow-md">
              <UserCheck className="h-4 w-4 mr-2" />Kelola Guru
            </TabsTrigger>
            <TabsTrigger value="siswa" className="rounded-lg px-4 py-2 text-sm data-[state=active]:bg-blue-600 data-[state=active]:text-white data-[state=active]:shadow-md">
              <Users className="h-4 w-4 mr-2" />Kelola Siswa
            </TabsTrigger>
            <TabsTrigger value="hasil" className="rounded-lg px-4 py-2 text-sm data-[state=active]:bg-blue-600 data-[state=active]:text-white data-[state=active]:shadow-md">
              <BarChart3 className="h-4 w-4 mr-2" />Hasil Ujian
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="ujian" className="mt-4 space-y-4">
                  ...
                  <Card className="clay-glass border-0">
                    ...
                    <CardContent className="space-y-4">
                      ...
                      <div className="flex gap-2">
                        <Button onClick={handleSaveExam} className="bg-blue-600 hover:bg-blue-700 text-white rounded-xl">{editingExam ? 'Update Ujian' : 'Simpan Ujian'}</Button>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
              ...
              <div className="lg:col-span-5 space-y-3">
                <div className="flex items-center gap-2 mb-1"><FileText className="h-4 w-4 text-blue-500" /><h3 className="text-sm font-bold text-gray-700">DAFTAR UJIAN</h3></div>
                {examLoading ? (
                  ...
                  <div className="space-y-3 max-h-[calc(100vh-320px)] overflow-y-auto custom-scrollbar pr-1">
                    {exams.map((exam) => (
                      <motion.div key={exam.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} layout>
                        <div className={`clay p-4 cursor-pointer transition-all hover:shadow-lg ${selectedExam?.id === exam.id ? 'ring-2 ring-blue-400 shadow-lg' : ''}`} onClick={() => handleSelectExam(exam)}>
                          ...
                          {exam.subject && <Badge className="bg-amber-100 text-amber-700 text-[10px] mb-2">{exam.subject}</Badge>}
                  ...
                  <div className="clay-glass p-5 border-0 space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                              ...
                              <div className="space-y-1">
                                ...
                                <Select value={qCorrect} onValueChange={setQCorrect}>
                                  <SelectTrigger className="neu-input bg-transparent h-9 text-sm"><SelectValue /></SelectTrigger>
                                  <SelectContent>{['A', 'B', 'C', 'D', 'E'].map((o) => (<SelectItem key={o} value={o}>Opsi {o}</SelectItem>))}</SelectContent>
                                </Select>
                              </div>
                            </div>
                    ...
                    {questions.length === 0 ? (
                      ...
                      <>
                        ...
                        <div className="flex items-center justify-between bg-gray-50/80 rounded-xl px-4 py-2.5">
                          ...
                          {selectedQuestions.size > 0 && (
                            <Button
                              ...
                              onClick={() => setDeleteDialog({ type: 'bulk', id: '', name: `${selectedQuestions.size} soal terpilih` })}
                            >
                        ...
                        <div className="space-y-3 max-h-[calc(100vh-540px)] overflow-y-auto custom-scrollbar pr-1">
                          {questions.map((q, idx) => (
                            <div key={q.id} className={`neu-flat p-4 transition-all ${selectedQuestions.has(q.id) ? 'ring-2 ring-blue-300 bg-blue-50/30' : ''}`}>
                              <div className="flex items-start gap-3">
                                <button onClick={() => toggleSelectQuestion(q.id)} className="mt-0.5 shrink-0">
                                  {selectedQuestions.has(q.id) ? <CheckSquare className="h-4 w-4 text-blue-600" /> : <Square className="h-4 w-4 text-gray-300 hover:text-gray-500" />}
                                </button>
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-2 mt-2">
                                    ...
                                    {q.explanation && <Badge variant="outline" className="text-xs text-gray-500">Pembahasan tersedia</Badge>}
                                  </div>
                                </div>
                                <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-blue-400 hover:text-blue-600 shrink-0" onClick={() => handleEditQuestion(q)}>
                                  <Pencil className="h-3.5 w-3.5" />
                                </Button>
                                <Button size="sm" variant="ghost" className="h-7 w-7 p-0 text-red-400 hover:text-red-600 shrink-0" onClick={() => setDeleteDialog({ type: 'question', id: q.id, name: `Soal ${idx + 1}` })}>
          ...
          {/* ==================== Kelola Guru ==================== */}
          <TabsContent value="guru" className="mt-4"><KelolaGuru /></TabsContent>

          {/* ==================== Kelola Siswa ==================== */}
          <TabsContent value="siswa" className="mt-4"><KelolaSiswa /></TabsContent>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-12 gap-3">
              ...
              <div className="sm:col-span-4">
                <Select value={resultExamFilter} onValueChange={setResultExamFilter}>
                  <SelectTrigger className="neu-input bg-transparent h-11 text-sm">
                    <SelectValue placeholder="Filter Paket Ujian" />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="sm:col-span-3">
                <div className="flex bg-gray-100 rounded-xl p-1 h-11 items-center gap-1">
                  <button
                    onClick={() => setResultGroupMode('grouped')}
                    className={`flex-1 text-xs font-semibold py-2 rounded-lg transition-all ${resultGroupMode === 'grouped' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                  >
                    Per Paket
                  </button>
                  <button
                    onClick={() => setResultGroupMode('flat')}
                    className={`flex-1 text-xs font-semibold py-2 rounded-lg transition-all ${resultGroupMode === 'flat' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                  >
                    Semua
                  </button>
                </div>
                  ...
                  Object.entries(groupedResults).map(([examId, examResults]) => {
                    ...
                    return (
                      <motion.div key={examId} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                        <div className="clay-glass overflow-hidden">
                          {/* Exam Header */}
                          <div className="bg-gradient-to-r from-blue-50 to-cyan-50 px-5 py-3 flex items-center justify-between">
                            <div>
                              <h3 className="font-bold text-gray-800 text-sm">{examInfo?.title || 'Ujian'}</h3>
                              <div className="flex items-center gap-3 text-xs text-gray-500 mt-0.5">
                                <span>{examResults.length} peserta</span>
                                <span>• Rata-rata: <span className="font-bold text-blue-600">{avgScore}</span></span>
                                <span>• Lulus: <span className="font-bold text-green-600">{passCount}</span>/{examResults.length}</span>
                              </div>
                            </div>
                            <Button size="sm" variant="outline" className="h-7 text-[11px] rounded-lg gap-1" onClick={() => handleExportResults(examId)}>
                          ...
                          <Table>
                            <TableHeader>
                              <TableRow>
                                <TableHead className="text-xs font-bold w-12">NO</TableHead>
